...
if __name__ == '__main__':
    ...
    recipe = VAE.ModelRecipe(
        id=RECIPE_ID,
        specification=VAE.ModelSpecification(
            generate_space=dev_ds_dummy.get_space_of_corresponding_normalized_vars(problem.generate_space),

        ),
        hyperparameters=VAE.ModelHyperparameters(
            ...
            encoder_spec=NetworkSpecification(layers=[
                Linear(problem.generate_space.n_dims, 16),
                ReLU(),
            ...
            loss_reconstruction_formulation=VAE.KEY_RECONSTRUCTION_MSE,
            loss_reconstruction_multiplier=1,
            latent_distribution=VAE.KEY_LATENT_DIST_STANDARD_NORMAL
        ),
        training_settings=VAE.TrainingSettings(
            batch_size=1600,
